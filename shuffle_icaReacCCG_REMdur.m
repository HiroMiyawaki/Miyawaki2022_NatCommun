function shuffle_icaReacCCG_REMdur(basename)
...
startIdx=1;
if exist([basename '-icaReacCCG_remDur_resume.mat'],'file')        
    load([basename '-icaReacCCG_remDur_sh.mat'])
    load([basename '-icaReacCCG_remDur_resume.mat'])
    
    oldParam=icaReacCCG_remDur_sh(1).param;
...
for tempIdx=startIdx:length(icaReac)
    ...
    for hcIdx=1:nHC
        ...
        nREMbin=sum(subState==5);
        for behState=1:length(behList)
            switch behList{behState}
                case 'entire'
                    X=subset;
                case 'wake'
                    X=subset(:,subState==1);
            ...
            fprintf('%s Template %d/%d, target %d/%d, behavior %d/%d of %s\n',...
                datestr(now),tempIdx,length(icaReac),hcIdx,nHC, behState,length(behList),sesName)
           parfor ite=1:nIte
                gpCcg=gpuArray(zeros(nTemplate*(nTemplate-1)/2,100*2+1));
                pairIdx=0;
                for n=1:nTemplate
    ...
    for behState=1:length(behList)
        ...
        icaReacCCG_remDur_sh(tempIdx).(behName).real.acg=tempACG{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).real.ccg=tempCCG{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).shuffle.mean=tempShMean{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).shuffle.ci95=tempShConf95{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).shuffle.global95= tempShGlobal95{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).shuffle.ci99=tempShConf99{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).shuffle.global99= tempShGlobal99{behState};
        icaReacCCG_remDur_sh(tempIdx).(behName).nBin=nBin{behState};
    end
    icaReacCCG_remDur_sh(tempIdx).pairID=pair;
    icaReacCCG_remDur_sh(tempIdx).region=icaReac(tempIdx).region(sigReac);
    icaReacCCG_remDur_sh(tempIdx).instReacID=sigReac;
    icaReacCCG_remDur_sh(tempIdx).tBinSize=tBinSize;
    icaReacCCG_remDur_sh(tempIdx).template=icaReac(tempIdx).tempName;
    icaReacCCG_remDur_sh(tempIdx).generator=mfilename;
    icaReacCCG_remDur_sh(tempIdx).generatedate=datestr(now,'yyyy-mm-dd');
    icaReacCCG_remDur_sh(tempIdx).param=param;
    
    save([basename '-icaReacCCG_remDur_sh.mat'],'icaReacCCG_remDur_sh','-v7.3')
    save([basename '-icaReacCCG_remDur_resume.mat'],'tempIdx','-v7.3')
end
        
delete([basename '-icaReacCCG_remDur_resume.mat'])        
