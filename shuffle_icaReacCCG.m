function shuffle_icaReacCCG(basename)
...
%%
startIdx=1;
if exist([basename '-icaReacCCG_resume.mat'],'file')        
    load([basename '-icaReacCCG_sh.mat'])
    load([basename '-icaReacCCG_resume.mat'])
    
    oldParam=icaReacCCG_sh(1).param;
...
%             for ite=1:nIte
    ...
    for behState=1:length(behList)
        ...
        icaReacCCG_sh(tempIdx).(behName).real.acg=tempACG{behState};
        icaReacCCG_sh(tempIdx).(behName).real.ccg=tempCCG{behState};
        icaReacCCG_sh(tempIdx).(behName).shuffle.mean=tempShMean{behState};
        icaReacCCG_sh(tempIdx).(behName).shuffle.ci95=tempShConf95{behState};
        icaReacCCG_sh(tempIdx).(behName).shuffle.global95= tempShGlobal95{behState};
        icaReacCCG_sh(tempIdx).(behName).shuffle.ci99=tempShConf99{behState};
        icaReacCCG_sh(tempIdx).(behName).shuffle.global99= tempShGlobal99{behState};
        icaReacCCG_sh(tempIdx).(behName).nBin=nBin{behState};
    end
    icaReacCCG_sh(tempIdx).pairID=pair;
    icaReacCCG_sh(tempIdx).region=icaReac(tempIdx).region(sigReac);
    icaReacCCG_sh(tempIdx).instReacID=sigReac;
    icaReacCCG_sh(tempIdx).tBinSize=tBinSize;
    icaReacCCG_sh(tempIdx).template=icaReac(tempIdx).tempName;
    icaReacCCG_sh(tempIdx).generator=mfilename;
    icaReacCCG_sh(tempIdx).generatedate=datestr(now,'yyyy-mm-dd');
    icaReacCCG_sh(tempIdx).param=param;
    
    save([basename '-icaReacCCG_sh.mat'],'icaReacCCG_sh','-v7.3')
    save([basename '-icaReacCCG_resume.mat'],'tempIdx','-v7.3')
end
        
delete([basename '-icaReacCCG_resume.mat'])        
