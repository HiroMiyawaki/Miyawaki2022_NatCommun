function shuffle_icaReacCCG_exHFO_baseCond(basename)
...
%%
startIdx=1;
if exist([basename '-icaReacCCG_exHFObaseCond_resume.mat'],'file')        
    load([basename '-icaReacCCG_exHFObaseCond_sh.mat'])
    load([basename '-icaReacCCG_exHFObaseCond_resume.mat'])
    
    oldParam=icaReacCCG_exHFObaseCond_sh(1).param;
...
for tempIdx=startIdx:2 %baseline/conditioning
    ...
    for behState=1:length(behList)
        ...
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).real.acg=tempACG{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).real.ccg=tempCCG{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).shuffle.mean=tempShMean{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).shuffle.ci95=tempShConf95{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).shuffle.global95= tempShGlobal95{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).shuffle.ci99=tempShConf99{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).shuffle.global99= tempShGlobal99{behState};
        icaReacCCG_exHFObaseCond_sh(tempIdx).(behName).nBin=nBin{behState};
    end
    icaReacCCG_exHFObaseCond_sh(tempIdx).pairID=pair;
    icaReacCCG_exHFObaseCond_sh(tempIdx).region=icaReac(tempIdx).region(sigReac);
    icaReacCCG_exHFObaseCond_sh(tempIdx).instReacID=sigReac;
    icaReacCCG_exHFObaseCond_sh(tempIdx).tBinSize=tBinSize;
    icaReacCCG_exHFObaseCond_sh(tempIdx).template=icaReac(tempIdx).tempName;
    icaReacCCG_exHFObaseCond_sh(tempIdx).generator=mfilename;
    icaReacCCG_exHFObaseCond_sh(tempIdx).generatedate=datestr(now,'yyyy-mm-dd');
    icaReacCCG_exHFObaseCond_sh(tempIdx).param=param;
    
    save([basename '-icaReacCCG_exHFObaseCond_sh.mat'],'icaReacCCG_exHFObaseCond_sh','-v7.3')
    save([basename '-icaReacCCG_exHFObaseCond_resume.mat'],'tempIdx','-v7.3')
end
        
delete([basename '-icaReacCCG_exHFObaseCond_resume.mat'])        
