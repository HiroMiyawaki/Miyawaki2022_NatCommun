function shuffle_icaReacCCG_dropHFO_baseCond(basename)
...
%%
startIdx=1;
if exist([basename '-icaReacCCG_dropHFO_baseCond_resume.mat'],'file')        
    load([basename '-icaReacCCG_dropHFO_baseCond_sh.mat'])
    load([basename '-icaReacCCG_dropHFO_baseCond_resume.mat'])
    
    oldParam=icaReacCCG_dropHFO_baseCond_sh(1).param;
...
for tempIdx=startIdx:2 %for baseline/conditioning
    ...
    for hcIdx=1:nHC
        ...
        hfoT=amyHFO.timestamps(amyHFO.timestamps(:,1)>sessions.homecage(hcIdx,1) & amyHFO.timestamps(:,2)<sessions.homecage(hcIdx,2),:);
        nHfo=size(hfoT,1);
        ...
        for behState=1:length(behList)
            ...
            parfor ite=1:nIte
                    shHfo=hfoT+(rand(nHfo,1)*diff(hfoJit)+hfoJit(1)).*(randi(2,nHfo,1)*2-3);
                    hfoBin=false(1,nSubBin);
                    for hfoIdx=1:nHfo
                        firstIdx=max(1,floor(shHfo(hfoIdx,1)/tBinSize)+1-tBinOffset);
                        lastIdx=min(nSubBin,ceil(shHfo(hfoIdx,2)/tBinSize)-tBinOffset);
                        hfoBin(firstIdx:lastIdx)=true;
                    end

                switch behState
                    case 1
                        toUse=subNrem&(~hfoBin);
                    otherwise
                        continue
                end
            ...
            if mod(behState,2)==1
                ...
                for hfoIdx=1:nHfo
                    firstIdx=max(0,floor(hfoT(hfoIdx,1)/tBinSize)+1-tBinOffset);
                    lastIdx=min(nSubBin,ceil(hfoT(hfoIdx,2)/tBinSize)-tBinOffset);
    ...
    for behState=1:length(behList)
        ...
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).real.acg=tempACG{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).real.ccg=tempCCG{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).shuffle.mean=tempShMean{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).shuffle.ci95=tempShConf95{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).shuffle.ci99=tempShConf99{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).shuffle.peak=tempPeak{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).shuffle.trough=tempTrough{behState};
        icaReacCCG_dropHFO_baseCond_sh(tempIdx).(behName).nBin=nBin{behState};
    end
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).pairID=pair;
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).region=icaReac(tempIdx).region(sigReac);
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).instReacID=sigReac;
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).tBinSize=tBinSize;
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).template=icaReac(tempIdx).tempName;
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).generator=mfilename;
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).generatedate=datestr(now,'yyyy-mm-dd');
    icaReacCCG_dropHFO_baseCond_sh(tempIdx).param=param;
    
    save([basename '-icaReacCCG_dropHFO_baseCond_sh.mat'],'icaReacCCG_dropHFO_baseCond_sh','-v7.3')
    save([basename '-icaReacCCG_dropHFO_baseCond_resume.mat'],'tempIdx','-v7.3')
end
        
delete([basename '-icaReacCCG_dropHFO_baseCond_resume.mat'])        
