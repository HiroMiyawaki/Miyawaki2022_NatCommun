function maxCh=fear_getMaxDeltaCh(basename,varargin)
...
param.candCh=64+(1:60);
param.lfp=fear_getLFPpath(basicMetaData.lfp);
param.deltaBand=[0.5,4];
param.wideBand=[0.5,20];
...
thetaPow=sum(meanPow(freq>min(param.deltaBand)&freq<max(param.deltaBand),:),1);
