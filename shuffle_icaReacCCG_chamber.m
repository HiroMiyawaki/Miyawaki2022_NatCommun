function shuffle_icaReacCCG_chamber(basename)
...
%%
[sesTime,sesNameList]=getChamberTime(sessions,cues);
%%
startIdx=1;
if exist([basename '-icaReacCCGchamber_resume.mat'],'file')        
    load([basename '-icaReacCCGchamber_sh.mat'])
    load([basename '-icaReacCCGchamber_resume.mat'])
    
    oldParam=icaReacCCGchamber_sh(1).param;
...
for tempIdx=startIdx:length(icaReac)
    ...
    for hcIdx=1:nHC
        ...
        if isempty(X)
            ...
            fprintf('%s Template %d/%d, target %d/%d of %s\n',...
                datestr(now),tempIdx,length(icaReac),hcIdx,nHC,sesName)
            parfor ite=1:nIte
        ...
        icaReacCCGchamber_sh(tempIdx).real.acg=tempACG;
        icaReacCCGchamber_sh(tempIdx).real.ccg=tempCCG;
        icaReacCCGchamber_sh(tempIdx).shuffle.mean=tempShMean;
        icaReacCCGchamber_sh(tempIdx).shuffle.ci95=tempShConf95;
        icaReacCCGchamber_sh(tempIdx).shuffle.global95= tempShGlobal95;
        icaReacCCGchamber_sh(tempIdx).shuffle.ci99=tempShConf99;
        icaReacCCGchamber_sh(tempIdx).shuffle.global99= tempShGlobal99;
        icaReacCCGchamber_sh(tempIdx).nBin=nBin;
    icaReacCCGchamber_sh(tempIdx).pairID=pair;
    icaReacCCGchamber_sh(tempIdx).region=icaReac(tempIdx).region(sigReac);
    icaReacCCGchamber_sh(tempIdx).instReacID=sigReac;
    icaReacCCGchamber_sh(tempIdx).tBinSize=tBinSize;
    icaReacCCGchamber_sh(tempIdx).template=icaReac(tempIdx).tempName;
    icaReacCCGchamber_sh(tempIdx).generator=mfilename;
    icaReacCCGchamber_sh(tempIdx).generatedate=datestr(now,'yyyy-mm-dd');
    icaReacCCGchamber_sh(tempIdx).param=param;
    
    save([basename '-icaReacCCGchamber_sh.mat'],'icaReacCCGchamber_sh','-v7.3')
    save([basename '-icaReacCCGchamber_resume.mat'],'tempIdx','-v7.3')
end
        
delete([basename '-icaReacCCGchamber_resume.mat'])        
