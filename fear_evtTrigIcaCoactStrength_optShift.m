function fear_evtTrigIcaCoactStrength_optShift(basename,varargin)
...
gap=icaReacStrWake_optShift.tShift(:,param.wakeIdx);
...
clear res
tMin=icaReacStrWake_optShift.targetTime(param.wakeIdx,1);
tMax=icaReacStrWake_optShift.targetTime(param.wakeIdx,2);

%load homecage of pre- and post- retention sessions to normalize power
tRange=[tMin,tMax];
typeMax=5;
behIdx=1; %awake
fileName='evtTrigIcaCoact-cueRet-opt.mat';
varName='evtTrigCoact';
fprintf('%s getting peri event triggered average in cue-ret, opt-shift \n',datestr(now))
