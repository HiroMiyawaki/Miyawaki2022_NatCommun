function fear_icaCoactTrigWaveletCueRet_optShift(basename,varargin)
...
param.targetSes=5;
param.varName='icaCoactTrigWavelet_optShift';

param.tWin=1.1; %gap can be -100 ~ 100 ms, so expand window
%%
param=parseParameters(param,varargin);
%%
load([basicMetaData.AnalysesName '-icaReacStrWake_optShift.mat'])
...
%%
tMin=icaReacStrWake_optShift.targetTime(param.targetSes,1);
tMax=icaReacStrWake_optShift.targetTime(param.targetSes,2);
    ...
    clear res
        ...
        if isempty(across)
            ...
            for idx=1:length(across)
                targetID=across(idx);
                tPos=coact.timestamps{param.targetSes,targetID};
            ...
            res.reacID =coact.pairID(across,:);
            res.tGap=coact.tShift(across,param.targetSes);
            res.sigLevel=coact.sigNREM(across);
        end
        res.t=tBin;
        res.f=frequency;
        res.param=param;
