function maxCh=fear_getMaxGammaCh(basename,varargin)
...
param.candCh=128+(1:60);
param.lfp=fear_getLFPpath(basicMetaData.lfp);
param.gammaBand=[50,100];
param.wideBand=[1,200];
...
gammaPow=sum(meanPow(freq>min(param.gammaBand)&freq<max(param.gammaBand),:),1);

ngCh=zscore(widePow)<-1.5;

gammaRatio=gammaPow./widePow;
