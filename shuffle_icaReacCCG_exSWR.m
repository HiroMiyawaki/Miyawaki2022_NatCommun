function shuffle_icaReacCCG_exSWR(basename)
...
%%
startIdx=1;
if exist([basename '-icaReacCCG_exSWR_resume.mat'],'file')        
    load([basename '-icaReacCCG_exSWR_sh.mat'])
    load([basename '-icaReacCCG_exSWR_resume.mat'])
    
    oldParam=icaReacCCG_exSWR_sh(1).param;
...
for tempIdx=startIdx:length(icaReac)
    ...
    for hcIdx=1:nHC
        ...
        subRipple=rippleBin(targetBin);
        subSpindle=spindleBin(targetBin);
        subset=icaReac(tempIdx).strength(sigReac,targetBin);
        
        for behState=1:length(behList)
            switch behState
                case 1
                    X=subset(:,subNrem&(~subRipple));
                case 2
                    X=subset(:,subNrem&(~subSpindle));
                case 3
                    X=subset(:,subNrem&(~subRipple)&(~subSpindle));
            end
    ...
    for behState=1:length(behList)
        ...
        icaReacCCG_exSWR_sh(tempIdx).(behName).real.acg=tempACG{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).real.ccg=tempCCG{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).shuffle.mean=tempShMean{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).shuffle.ci95=tempShConf95{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).shuffle.global95= tempShGlobal95{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).shuffle.ci99=tempShConf99{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).shuffle.global99= tempShGlobal99{behState};
        icaReacCCG_exSWR_sh(tempIdx).(behName).nBin=nBin{behState};
    end
    icaReacCCG_exSWR_sh(tempIdx).pairID=pair;
    icaReacCCG_exSWR_sh(tempIdx).region=icaReac(tempIdx).region(sigReac);
    icaReacCCG_exSWR_sh(tempIdx).instReacID=sigReac;
    icaReacCCG_exSWR_sh(tempIdx).tBinSize=tBinSize;
    icaReacCCG_exSWR_sh(tempIdx).template=icaReac(tempIdx).tempName;
    icaReacCCG_exSWR_sh(tempIdx).generator=mfilename;
    icaReacCCG_exSWR_sh(tempIdx).generatedate=datestr(now,'yyyy-mm-dd');
    icaReacCCG_exSWR_sh(tempIdx).param=param;
    
    save([basename '-icaReacCCG_exSWR_sh.mat'],'icaReacCCG_exSWR_sh','-v7.3')
    save([basename '-icaReacCCG_exSWR_resume.mat'],'tempIdx','-v7.3')
end
        
delete([basename '-icaReacCCG_exSWR_resume.mat'])        
